...
class Workspace:
    def __init__(self, cfg):
        ...
        temp_agent_name = cfg.wandb_group.split('_')[1]
        self.agent_name = temp_agent_name.replace("+", "_")
        print("change {} to {}".format(temp_agent_name, self.agent_name))
    ...
    def eval_adroit(self, force_number_episodes=None, do_log=True):
        ...
        for i in tqdm(range(n_eval_episode)):
            ...
            self.video_recorder.init(self.eval_env, enabled=(episode <= 9))
            while not time_step.last():
                if self.agent_name in ['pieg', 'piegcycaug']:
                    # print(self.agent, 'acts as pieg or piegcycaug')
                    with torch.no_grad():
            ...
            # tqdm.write('eval_adroit: {}_{}.mp4'.format(i, self.cfg.wandb_group))
            self.video_recorder.save('{}_{}.mp4'.format(i, self.cfg.wandb_group))
