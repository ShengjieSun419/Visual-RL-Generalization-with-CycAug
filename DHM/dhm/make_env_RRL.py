            ...
            if not self.test_image:
                for cam in self.cam_list:  # for each camera, render once
                    ...
                    if self.cfg_dict['task']['background']['type'] == 'video' and self.cfg_dict['task'][
                        ...
                        # img = (1 - idx) * img + idx * self.video_buf[self.step_count % self.video_len, :,
                        #                               :self.cfg_dict['image_height'], :self.cfg_dict['image_width']]
                        img = (1 - idx) * img + idx * cv2.resize(self.video_buf[self.step_count % self.video_len, :, :, :].transpose(1, 2, 0), (self.cfg_dict['image_width'], self.cfg_dict['image_height'])).transpose(2, 0, 1)
                    elif self.cfg_dict['task']['background']['type'] == 'color':
                        pass
                    self.step_count += 1
...
def make_env_RRL(env_name, test_image=False, cam_list=None,
    ...
    if cfg_dict['task']['mode'] == 'test':
        if level != 'original':
            ...
            cfg_dict['task']['light']['position'] = difficulty
            cfg_dict['task']['light']['intensity'] = difficulty
            cfg_dict['task']['light']['color'] = difficulty
