...
parser.add_argument('--type', default='original', type=str)
parser.add_argument('--difficulty', default='easy', type=str)
parser.add_argument('--intensity', default=0, type=float)
parser.add_argument('--is_distracting_cs', default='False', type=str)
...
def evaluate(env, agent, video, video_dir, mode, num_episodes, seed=0, domain_name='cartpole', task_name='swingup', algo='svea', step=int(5e5)):
    ...
    for i in tqdm(range(num_episodes)):
        ...
        while not done:
            count += 1
            if algo in ['pieg', 'pieg_cycaug']:
                # print(algo, 'acts as pieg or pieg_cycaug')
                with torch.no_grad():
        ...
        video.save(f'eval_{args.type}_{args.difficulty}_{args.intensity}_{i}.mp4')
...
def main(args):
    ...
    if args.is_distracting_cs.lower() == 'false':
        args.is_distracting_cs = False
    elif args.is_distracting_cs.lower() == 'true':
        args.is_distracting_cs = True
    else:
        raise argparse.ArgumentTypeError("is_distracting_cs Invalid value. Please use 'True' or 'False'.")
    env = make_env(domain_name=domain_name, task_name=task_name, seed=args.seed, type=args.type, difficulty=args.difficulty, intensity=args.intensity, is_distracting_cs=args.is_distracting_cs)
    ...
    if algorithm not in ['pieg', 'pieg_cycaug']:
